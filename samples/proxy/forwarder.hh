...
public:
    ...
    public:
        ...
        void per_flow_enqueue(rte_packet pkt,process_type type) {
            ...
            if(likely(type==process_type::hybernate)){
                ...
                /*if(_f._pkt_counter>=FLAGS_batch_size){
                     _f._pkt_counter=0;
    ...
    void dispath_flow(rte_packet pkt){
        ...
        if(ntohs(eth_h->ether_type) == static_cast<uint16_t>(eth_protocol_num::ipv4)) {
            ...
            if(ip_h->protocol == static_cast<uint8_t>(ip_protocol_num::udp)) {
                ...
                if(_pkt_counter>=FLAGS_batch_size){
                     _pkt_counter=0;
                     _batch.current_idx=!_batch.current_idx;
                     _batch.schedule_task(!_batch.current_idx);
                 }
    ...
    public:
        ...
        void compute_parameter(){
            ...
            _parameters.cpu_enqueue_rate = _profile_elements.cpu_enqueue_time/FLAGS_batch_size;
        ...
        double compute_cpu_time(uint64_t pkt_num){
            ...
            return _parameters.cpu_process_rate*pkt_num+FLAGS_batch_size*_parameters.cpu_enqueue_rate;
        ...
        bool need_periodical_profile(){
            if(_profileing||_period_profile||FLAGS_batch_size==1){
                return false;
            }
            _period_profile_num ++;
        ...
        void schedule_task(uint64_t index){
        	...
        	if(unlikely(_profile_num<100)){
        		...
        		if(unlikely(_profileing&&lcore_id ==0&& FLAGS_dynamic_adjust)){
            ...
            //adjust_enqueue_rate();
            if(FLAGS_print_simple_time) printf("locre: %d,Simple Enqueuing time: %f\n", lcore_id,static_cast<double>(simple_elapsed.count() / 1.0));
            ...
            if(FLAGS_print_time) printf("lcore %d,Enqueuing time: %f\n",lcore_id, static_cast<double>(elapsed.count() / 1.0));
            ...
            if(FLAGS_batch_size!=1){
                ...
                if(FLAGS_print_time)printf("lcore %d,partition: %d\n",lcore_id, partition);
                //partition=_flows[index].size();
                if(FLAGS_print_time)std::cout<<"lcore_id: "<<lcore_id<<"Total flow_num:"<<_flows[index].size()<<std::endl;
                if(FLAGS_print_time)printf("lcore %d,partition: %d\n",lcore_id, partition);
            }
            assert(partition!=-1);
            ...
            if(FLAGS_print_time)printf("lcore %d ,Scheduling time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
            started[lcore_id] = steady_clock_type::now();


            simple_stoped[lcore_id] = steady_clock_type::now();
            simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
            if(FLAGS_print_simple_time)    printf("lcore: %d, simple schedule time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
            ...
            if(partition>0) {
                ...
                if(FLAGS_print_time)std::cout<<"ngpu_pkts:"<<ngpu_pkts/sizeof(PKT)<<std::endl;
...
//#pragma omp parallel for
                ...
                stoped[lcore_id] = steady_clock_type::now();
                elapsed = stoped[lcore_id] - started[lcore_id];
                if(FLAGS_print_time)printf("lcore %d batch pkt time: %f\n",lcore_id, static_cast<double>(elapsed.count() / 1.0));
                started[lcore_id] = steady_clock_type::now();

                simple_stoped[lcore_id] = steady_clock_type::now();
                simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
                if(FLAGS_print_simple_time)    printf("lcore: %d, simple batch pkt time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
                ...
                if(_flows[!index].empty()==false){
                    ...
                    stoped[lcore_id] = steady_clock_type::now();
                    elapsed = stoped[lcore_id] - started[lcore_id];
                    if(FLAGS_print_time)  printf("lcore: %d, Sync time: %f\n",lcore_id, static_cast<double>(elapsed.count() / 1.0));
                    started[lcore_id] = steady_clock_type::now();


                    simple_stoped[lcore_id] = steady_clock_type::now();
                    simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
                    if(FLAGS_print_simple_time)    printf("lcore: %d, simple sync time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
                    ...
                    stoped[lcore_id] = steady_clock_type::now();
                    elapsed = stoped[lcore_id] - started[lcore_id];
                    if(FLAGS_print_time)  printf("lcore: %d,Copyback time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
                    started[lcore_id] = steady_clock_type::now();
                    ...
                    stoped[lcore_id] = steady_clock_type::now();
                    elapsed = stoped[lcore_id] - started[lcore_id];
                    if(FLAGS_print_time)  printf("lcore: %d,memset time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
                    started[lcore_id] = steady_clock_type::now();

                    simple_stoped[lcore_id] = steady_clock_type::now();
                    simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
                    if(FLAGS_print_simple_time)    printf("lcore: %d, simple copyback time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
                  ...
                  //  gpu_states[!index]=nullptr;
                    stoped[lcore_id] = steady_clock_type::now();
                    elapsed = stoped[lcore_id] - started[lcore_id];
                    if(FLAGS_print_time)  printf("lcore: %d,batch unmap time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
                    started[lcore_id] = steady_clock_type::now();
                    ...
                    if(FLAGS_print_simple_time) printf("lcore: %d,batch unmap time: %f\n", lcore_id,static_cast<double>(el.count() / 1.0));
                   ...
                   /* if(gpu_pkts[!index]){
                    ...
                    stoped[lcore_id] = steady_clock_type::now();
                    elapsed = stoped[lcore_id] - started[lcore_id];
                    if(FLAGS_print_time)  printf("lcore: %d,forward pkts time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
                    started[lcore_id] = steady_clock_type::now();

                    simple_stoped[lcore_id] = steady_clock_type::now();
                    simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
                    if(FLAGS_print_simple_time)    printf("lcore: %d, simple forward pkt time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
                }
...
//#pragma omp parallel for
                ...
                stoped[lcore_id] = steady_clock_type::now();
                elapsed = stoped[lcore_id] - started[lcore_id];
                if(FLAGS_print_time)printf("lcore: %d,Batching state time: %f\n",lcore_id, static_cast<double>(elapsed.count() / 1.0));
                started[lcore_id] = steady_clock_type::now();

                simple_stoped[lcore_id] = steady_clock_type::now();
                simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
                if(FLAGS_print_simple_time)    printf("lcore: %d, simple batching states time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));

                if(FLAGS_gpu_time||_profileing||_period_profile){
                    gpu_sync(stream);
                    started[lcore_id] = steady_clock_type::now();
                }
                ...
                if(FLAGS_gpu_time||_profileing||_period_profile){
                    ...
                    stoped[lcore_id] = steady_clock_type::now();
                    elapsed = stoped[lcore_id] - started[lcore_id];
                    if(FLAGS_gpu_time)    printf("lcore %d copy pkt to device time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
                    started[lcore_id] = steady_clock_type::now();

                    simple_stoped[lcore_id] = steady_clock_type::now();
                    simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
                    if(FLAGS_print_simple_time)    printf("lcore: %d, simple copy pkt to device time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
                ...
                stoped[lcore_id] = steady_clock_type::now();
                elapsed = stoped[lcore_id] - started[lcore_id];
                if(FLAGS_print_time)printf("lcore %d Memcpy to device time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));

                simple_stoped[lcore_id] = steady_clock_type::now();
                simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
                if(FLAGS_print_simple_time)    printf("lcore: %d, simple Memcpy to device time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
                ...
                stoped[lcore_id] = steady_clock_type::now();
                elapsed = stoped[lcore_id] - started[lcore_id];
                if(FLAGS_print_time) printf("lcore %d gpu launch time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
                started[lcore_id] = steady_clock_type::now();

                if(FLAGS_print_time)  printf("lcore_id: %d gpu launced just now\n", lcore_id);
                if(FLAGS_gpu_time||_profileing||_period_profile){
                    gpu_sync(stream);
                    stoped[lcore_id] = steady_clock_type::now();
                    elapsed = stoped[lcore_id] - started[lcore_id];
                    if(FLAGS_gpu_time) printf("lcore %d sync time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
                    started[lcore_id] = steady_clock_type::now();
                ...
                stoped[lcore_id] = steady_clock_type::now();
                elapsed = stoped[lcore_id] - started[lcore_id];
                if(FLAGS_print_time)  printf("lcore_id: %d Memcpy device to host time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
                started[lcore_id] = steady_clock_type::now();

                simple_stoped[lcore_id] = steady_clock_type::now();
                simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
                if(FLAGS_print_simple_time)    printf("lcore: %d, simple memcpy device to host time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
            ...
            stoped[lcore_id] = steady_clock_type::now();
            elapsed = stoped[lcore_id] - started[lcore_id];
            if(FLAGS_print_time)printf("lcore: %d,CPU processing time: %f\n", lcore_id,static_cast<double>(elapsed.count() / 1.0));
            started[lcore_id] = steady_clock_type::now();

            simple_stoped[lcore_id] = steady_clock_type::now();
            simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
            if(FLAGS_print_simple_time)    printf("lcore: %d, simple cpu processing time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
            ...
            simple_stoped[lcore_id] = steady_clock_type::now();
            simple_elapsed = simple_stoped[lcore_id] - simple_started[lcore_id];
            if(FLAGS_print_simple_time)    printf("lcore: %d, Task schedule time: %f\n",lcore_id, static_cast<double>(simple_elapsed.count() / 1.0));
            simple_started[lcore_id] = steady_clock_type::now();
        ...
        uint64_t get_partition(uint64_t index){
            ...
            std::cout<<std::endl;*/
            if(!FLAGS_schedule) return _flows[index].size();
            if(_profileing||_period_profile){
                ...
                _profile_elements.gpu_total_pkt_num = FLAGS_batch_size - _profile_elements.cpu_total_pkt_num ;
            ...
            for(int i=_flows[index].size();i>=0;i--){
                ...
                _gpu_pkt_num = FLAGS_batch_size - cpu_pkt_num;
                ...
                if(processing_time>=min_processing_time){
                    ...
                    if(FLAGS_print_time)std::cout<<"lcore_id: "<<lcore_id<<"cpu_pkts_processed: "<<pre_cpu_processing_num<<std::endl;
                    if(FLAGS_print_time)std::cout<<"lcore_id: "<<lcore_id<<"caculated cpu processed time: "<<cpu_time<<std::endl;
                    if(FLAGS_print_time)std::cout<<"lcore_id: "<<lcore_id<<"caculated gpu processed time: "<<_gpu_time<<std::endl;
                    if(FLAGS_print_time)printf("lcore: %d,CPU rate: %f\n", lcore_id,_parameters.cpu_process_rate);
                    if(FLAGS_print_time)printf("lcore: %d,GPU rate: %f\n", lcore_id,_parameters.gpu_process_rate);
                    if(FLAGS_print_time)printf("lcore: %d,enqueue rate: %f\n", lcore_id,_parameters.cpu_enqueue_rate);


                    if(i==0){
                        if(FLAGS_print_time||FLAGS_gpu_time)    std::cout<<"lcore_id: "<<lcore_id<<"GPU_max_pkt: "<<0<<std::endl;
                        return 0;
                    }else{
                        if(FLAGS_print_time||FLAGS_gpu_time)   std::cout<<"lcore_id: "<<lcore_id<<"GPU_max_pkt: "<<_flows[index][i]->packets[index].size()<<std::endl;
                        return i+1;
                    }
