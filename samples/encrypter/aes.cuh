...
__device__ const uint8_t rsbox[256] = {
...
public:
    ...
    __device__ inline void init_automataState(struct aes_flow_state& state){
    ...
    */
    ...
    __device__ static void AES_init_ctx(struct AES_ctx* ctx, const uint8_t* key)
    {
      KeyExpansion(ctx->RoundKey, key);
    }
    #if (defined(CBC) && (CBC == 1)) || (defined(CTR) && (CTR == 1))
    __device__ static void AES_init_ctx_iv(struct AES_ctx* ctx, const uint8_t* key, const uint8_t* iv)
    {
      KeyExpansion(ctx->RoundKey, key);
      memcpy (ctx->Iv, iv, AES_BLOCKLEN);
    }
    __device__ static void AES_ctx_set_iv(struct AES_ctx* ctx, const uint8_t* iv)
    {
      memcpy (ctx->Iv, iv, AES_BLOCKLEN);
    }
    #endif
    ...
    __device__ static void AddRoundKey(uint8_t round,state_t* state,uint8_t* RoundKey)
    {
      uint8_t i,j;
      for (i = 0; i < 4; ++i)
      {
    ...
    __device__ static void Cipher(state_t* state, uint8_t* RoundKey)
    {
      uint8_t round = 0;
    ...
    __device__ static void InvCipher(state_t* state,uint8_t* RoundKey)
    {
      uint8_t round = 0;
    ...
    __device__ static void AES_ECB_encrypt(struct AES_ctx *ctx, uint8_t* buf)
    {
      // The next function call encrypts the PlainText with the Key using AES algorithm.
      Cipher((state_t*)buf, ctx->RoundKey);
    }

    __device__ static void AES_ECB_decrypt(struct AES_ctx* ctx, uint8_t* buf)
    {
    ...
    __device__ static void XorWithIv(uint8_t* buf, uint8_t* Iv)
    {
      uint8_t i;
    ...
    __device__ static void AES_CBC_encrypt_buffer(struct AES_ctx *ctx,uint8_t* buf, uint32_t length)
    {
      uintptr_t i;
      uint8_t *Iv = ctx->Iv;
    ...
    __device__ static void AES_CBC_decrypt_buffer(struct AES_ctx* ctx, uint8_t* buf,  uint32_t length)
    {
      uintptr_t i;
    ...
    __device__ static void AES_CTR_xcrypt_buffer(struct AES_ctx* ctx, uint8_t* buf, uint32_t length)
    {
