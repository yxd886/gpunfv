#ifndef FLOW_MONITOR_HH
#define FLOW_MONITOR_HH
...
public:
	...
	inline void nf_init() {
		...
		info_for_gpu = init_flow_monitor_info(rules.size(), &rules[0]);
	}
	...
	inline void init_automataState(flow_monitor_flow_state &state) {
		state.match_no = 0;
		state.drop_no = 0;
		state.pass_no = 0;
	...
	inline void nf_logic(void *pkt, flow_monitor_flow_state *state) {	// specify nf logic function
	    process(pkt, state);
	}

private:
	inline void process(void *packet, flow_monitor_flow_state* state) {
		packetInfo info;
	...
	void match_rules(packetInfo *info, flow_monitor_flow_state* state) {
