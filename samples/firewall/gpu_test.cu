...
void *gpu_init(uint32_t size, void *ptr) {
	Infos info(size, ptr);
	void *devPtr;

	if(info.data_size) {
		gpu_malloc(&devPtr, info.data_size);
		checkCudaErrors(cudaMemcpy(devPtr, ptr, info.data_size, cudaMemcpyHostToDevice));

		*(info.data()) = devPtr;
	}

	gpu_malloc(&devPtr, sizeof(info));
	checkCudaErrors(cudaMemcpy(devPtr, &info, sizeof(info), cudaMemcpyHostToDevice));
